...
class IP(Structure):
    ...
    # __init__(self, *args, **kwargs) 创建完对象后调用，对当前对象的实例的一些初始化，无返回值,
    # 即在调用__new__之后，根据返回的实例初始化；
    # 注意，这里的第一个参数是self即对象本身【注意和new的区别】
...
class ICMP(Structure):
    ...
    def __init__(self, socket_bufffer):
        pass
...
sniffer.bind((host, 0))  # 这里端口为0，应该是监听所有的端口
...
try:
    while True:
        ...
        if ip_header.protocol == "ICMP":
            ...
            # ihl是头部长度,代表32位(即4字节)长的分片的个数 [我的理解是因为一个字节表示一个符号,
            # 所以这里的offset要搞成以字节为单位的,为的是下一句的提取数据]
